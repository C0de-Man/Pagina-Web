...
export default function PeliculasDeUsuario() {
  ...
  useEffect(() => {
    ...
    fetch(`${API_URL}/users/${encodeURIComponent(username)}/watched?tipo=PELICULA`, {
  ...
  const { valor: orden, setValor: setOrden, cargado: ordenCargado } = useSortPreference('user-watched', {
    campo: 'fechaEstreno',
    direccion: 'DESC',
  });
  ...
  return (
    <main className="min-h-screen bg-[#14181c] text-white font-sans">
      <div className="max-w-6xl mx-auto px-4 sm:px-6 lg:px-8 py-10">
        ...
        {peliculas.length === 0 ? (
          <p className="text-gray-500 text-sm">{username} hasn't marked any movies as watched yet.</p>
        ) : (
