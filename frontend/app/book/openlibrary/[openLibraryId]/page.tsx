...
export default async function BookOpenLibraryResolver({ params }: { params: Promise<{ openLibraryId: string }> }) {
  const { openLibraryId } = await params;

  const res = await fetch('${process.env.NEXT_PUBLIC_API_URL}/media/openlibrary', {
    method: 'POST',
