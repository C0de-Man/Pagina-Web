...
// Igual que en MovieFiltersSidebar, pero en TMDB discover/tv "with_runtime"
// filtra por duración de EPISODIO, no de la serie completa — no existe un
// filtro de "duración total" con series. Franjas ajustadas a episodio.
const DURACIONES = [
  { valor: 'corta', label: 'Short', hint: '<30 min/ep' },
  { valor: 'media', label: 'Medium', hint: '30–59 min/ep' },
  { valor: 'larga', label: 'Long', hint: '60+ min/ep' },
];

export default function SeriesFiltersSidebar({ currentYear }: { currentYear: number }) {
  ...
  return (
    <aside className="w-full lg:w-64 flex-shrink-0 bg-[#1c2228] border border-gray-800 rounded-lg p-4 h-fit space-y-6">
      ...
      <div>
        <p className="text-sm font-bold text-gray-300 uppercase tracking-wide mb-3">Episode duration</p>
        <div className="flex flex-col gap-2">
          {DURACIONES.map((d) => (
            <button
              key={d.valor}
